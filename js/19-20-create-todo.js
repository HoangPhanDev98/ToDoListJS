...
function createTodoElement(todo, params) {
  ...
  if (markAsDoneButton) {
    markAsDoneButton.addEventListener('click', function () {
      const newStatus = todoElement.dataset.status === 'pending' ? 'completed' : 'pending';
      ...
      const newButtonColor = newStatus === 'pending' ? 'btn-success' : 'btn-secondary';
  ...
  const buttonColor = currentStatus === 'pending' ? 'btn-success' : 'btn-secondary';
...
function populateTodoForm(todo) {
  ...
  if (todoCheckInput) {
    todo.status === 'pending' ? (todoCheckInput.checked = false) : (todoCheckInput.checked = true);
  }
  // set todoText input
}
...
function handleTodoFormSubmit(event) {
  ...
  const todoStatus = todoCheckboxInput.checked === false ? 'pending' : 'completed';
